...
namespace mt {
template <class T>
struct TensorWrap<T, ct::EnableIf<ct::IsBase<ct::Base<ct::TArrayViewTag>,
                                             ct::Derived<T>>::value>,
                  2> {
  ...
  static Tensor<value_type, 1> wrap(T &data) {
    return Tensor<value_type, 1>(data.data(), data.size());
  }
};

template <class T>
struct TensorWrap<const T, ct::EnableIf<ct::IsBase<ct::Base<ct::TArrayViewTag>,
                                                   ct::Derived<T>>::value>,
                  3> {
  using value_type = typename T::value_type;
...
namespace ct {
namespace ext {
template <class T, class Tag> struct ScalarComponent {
  using DType = T;
  template <class... ARGS>
  ...
  template <class U> operator U() const { return m_value; }
...
template <class T, class E = void> struct DataDimensionality {
...
struct DataDimensionality<
    T, EnableIf<IsBase<Base<TArrayViewTag>, Derived<T>>::value>> {
...
template <class T_> struct DataTableStorage {
  ...
  auto operator[](size_t idx)
      -> decltype(std::declval<mt::Tensor<T, storage_dim>>()[idx]) {
    T *ptr = m_data.data();
  ...
  auto operator[](size_t idx) const
      -> decltype(std::declval<mt::Tensor<const T, storage_dim>>()[idx]) {
  ...
  template <uint8_t I> void resizeSubarray(mt::Shape<I>) {
  ...
  void erase(uint32_t index) {
    if (data_dim > 1) {
      ...
      m_data.erase(m_data.begin() + index * subdim_size,
                   m_data.begin() + (index + 1) * subdim_size);
    } else {
...
template <class... Ts> struct DefaultStoragePolicy {
  using type = std::tuple<DataTableStorage<Ts>...>;
  type m_data;

  static type init() { return {}; }

  template <index_t I> auto get() -> decltype(std::get<I>(m_data)) {
    return std::get<I>(m_data);
  }

  template <index_t I> auto get() const -> decltype(std::get<I>(m_data)) {
    return std::get<I>(m_data);
  }

  DefaultStoragePolicy() {
    static_assert(
        std::is_lvalue_reference<decltype(this->template get<0>())>::value,
        "Expect to be returning a reference");
  }
};

template <class... Ts> struct SharedPtrStoragePolicy {
...
private:
  static void init(type &data, Indexer<0>) {
    using Ptr_t =
        typename std::remove_reference<decltype(std::get<0>(data))>::type;
    using Storage_t = typename Ptr_t::element_type;
    std::get<0>(data) = std::make_shared<Storage_t>();
  }
  template <index_t I> static void init(type &data, Indexer<I> idx) {
    using Ptr_t =
        typename std::remove_reference<decltype(std::get<I>(data))>::type;
    using Storage_t = typename Ptr_t::element_type;
...
template <class T> struct ReflectImpl<ext::DataTableStorage<T>, void> {
  ...
  static std::array<size_t, 2> shape(const DataType &data) {
...
template <uint8_t N> struct ReflectImpl<mt::Shape<N>, void> {
